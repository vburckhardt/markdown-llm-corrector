...
The IBM Cloud Framework for Financial Services requires an inbound network-based allowlist in front of the IBM Cloud Object Storage instance. The module's `bucket_cbr_rules` and `instance_cbr_rules` variables can be used to create a narrow context-based restriction rule that is scoped to the IBM Cloud Storage instance. Since CRA does not support checking for context-based restrictions, you can ignore the failing rule after setting the context-based restrictions.
